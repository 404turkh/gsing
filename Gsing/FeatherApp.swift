...
struct FeatherApp: App {
	...
	private func _handleURL(_ url: URL) {
		if url.scheme == "gsing" {
            ...
            if url.pathExtension == "gsing" {
                UIAlertController.showAlertWithOk(title: .localized("Error"), message: .localized("Gsing certificate file (.gsing) is now unsupported from v1.5.1, please refer to use .p12 and .mobileprovision instead."))
            }
		}
	}
...
class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    private func _createPipeline() {
        ...
        let pipeline = ImagePipeline {
            ...
            let dataCache = try? DataCache(name: "com.gsing.app.datacache") // disk cache
